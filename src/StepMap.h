...
public:
	 ...
	 *  @return ステップメモリの参照
	 */
	inline step_t& getStep(const Maze::Vector& v, const enum Purpose& sp = Goal) { return getStep(v.x, v.y, sp); }
	 ...
	 *  @param v ハイライト区画
	 */
	void print(const enum Purpose& sp = Goal, const Maze::Vector& v = Maze::Vector(-1,-1), const Maze::Dir& d = Maze::Dir::AbsMax) const {
		printf("\n");
		for(int8_t y=MAZE_SIZE-1; y>=0; y--){
			for(uint8_t x=0; x<MAZE_SIZE; x++)
			printf("+%s" C_RESET, maze.isKnown(x,y,Maze::Dir::North) ? (maze.isWall(x,y,Maze::Dir::North)?"---":"   ") : C_RED " - ");
			printf("+\n");
			for(uint8_t x=0; x<MAZE_SIZE; x++){
				printf("%s" C_RESET, maze.isKnown(x,y,Maze::Dir::West) ? (maze.isWall(x,y,Maze::Dir::West)?"|":" ") : C_RED ":");
				if(v==Maze::Vector(x, y)){
					printf(" %s%c " C_RESET, C_YELLOW, v==Maze::Vector(x,y)?(">^<vX"[d]):' ');
				}else{
					printf("%s%3d" C_RESET, v==Maze::Vector(x,y)?C_YELLOW:C_CYAN, stepMap[sp][y][x]);
				}
			}
			printf("%s" C_RESET, maze.isKnown(MAZE_SIZE-1,y,Maze::Dir::East) ? (maze.isWall(MAZE_SIZE-1,y,Maze::Dir::East)?"|":" ") : C_RED ":");
			printf("\n");
		}
		for(uint8_t x=0; x<MAZE_SIZE; x++)
		printf("+%s" C_RESET, maze.isKnown(x,0,Maze::Dir::South) ? (maze.isWall(x,0,Maze::Dir::South)?"---":"   ") : C_RED " - ");
		printf("+\n");
	}
	/** @function update
	*  @brief ステップマップの更新
	...
	*/
	void update(const std::vector<Maze::Vector>& dest, const enum Purpose& sp, const bool& onlyCanGo = false){
		// 全区画のステップを最大値に設定
		for(uint8_t y=0; y<MAZE_SIZE; y++)
		for(uint8_t x=0; x<MAZE_SIZE; x++)
		getStep(x, y, sp) = MAZE_STEP_MAX;
		// となりの区画のステップが更新されたので更新が必要かもしれない区画のキュー
		std::queue<Maze::Vector> q;
		...
		while(!q.empty()){
			// 注目する区画を取得
			Maze::Vector focus = q.front(); q.pop();
			step_t focus_step = getStep(focus, sp);
			// 4方向更新がないか調べる
			for(const auto& d: Maze::Dir::All()){
				Maze::Vector next = focus.next(d); //< となりの区画のステップを取得
