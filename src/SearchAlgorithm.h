	...
	public:
		...
		*/
		...
		goal(goal) {}
			...
			*/
			bool calcNextDirs(enum State& state, const Vector& pv, const Dir& pd, std::vector<Dir>& nextDirs, std::vector<Dir>& nextDirsInAdvance) {
				if(state == IDOLE){
					stepMapGoal.reset();
					stepMapStart.reset();
				...
				if(state == SEARCHING_FOR_GOAL){
					...
					if(std::find(goal.begin(), goal.end(), pv) != goal.end()){
						...
						if(!calcNextDirsByStepMap(stepMapGoal, pv, pd, nextDirs, nextDirsInAdvance)) state = GOT_LOST;
					}
				}
				...
				if(state == REACHED_GOAL){
					...
					if(candidates.empty()){
						...
						if(!calcNextDirsByStepMap(stepMapCandidates, pv, pd, nextDirs, nextDirsInAdvance)) state = GOT_LOST;
					}
				}
				...
				if(state == BACKING_TO_START){
					if(pv == start) {
						...
						if(!calcNextDirsByStepMap(stepMapStart, pv, pd, nextDirs, nextDirsInAdvance)) state = GOT_LOST;
					}
				}
