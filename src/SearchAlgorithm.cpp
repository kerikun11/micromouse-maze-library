...
bool SearchAlgorithm::updateWall(const State &state, const Vector &v, const Dir &d, const bool left, const bool front, const bool right, const bool back)
{
	bool result = true;
...
bool SearchAlgorithm::updateWall(const State &state, const Vector &v, const Dir &d, const bool &b)
{
...
bool SearchAlgorithm::findNextDir(const State state, const Vector v, const Dir d, const Dirs &nextDirCandidates, Dir &nextDir) const
{
	return findNextDir(state == IDENTIFYING_POSITION ? idMaze : maze, v, d, nextDirCandidates, nextDir);
}
bool SearchAlgorithm::findNextDir(const Maze &maze, const Vector v, const Dir d, const Dirs &nextDirCandidates, Dir &nextDir) const
{
	// 候補の中で行ける方向を探す
...
bool SearchAlgorithm::calcShortestDirs(Dirs &shortestDirs, const bool diagonal)
{
...
void SearchAlgorithm::printMap(const State state, const Vector vec, const Dir dir) const
{
	if (state == IDENTIFYING_POSITION)
...
int SearchAlgorithm::countIdentityCandidates(const WallLogs idWallLogs, Vector &ans) const
{
	int cnt = 0;
...
enum SearchAlgorithm::Status SearchAlgorithm::calcNextDirsSearchForGoal(const Vector &cv, const Dir &cd, Dirs &nextDirsKnown, Dirs &nextDirCandidates)
{
	Vectors candidates;
...
enum SearchAlgorithm::Status SearchAlgorithm::calcNextDirsSearchAdditionally(const Vector &cv, const Dir &cd, Dirs &nextDirsKnown, Dirs &nextDirCandidates)
{
	Vectors candidates;
...
enum SearchAlgorithm::Status SearchAlgorithm::calcNextDirsBackingToStart(const Vector &cv, const Dir &cd, Dirs &nextDirsKnown, Dirs &nextDirCandidates)
{
...
enum SearchAlgorithm::Status SearchAlgorithm::calcNextDirsGoingToGoal(const Vector &cv, const Dir &cd, Dirs &nextDirsKnown, Dirs &nextDirCandidates)
{
...
enum SearchAlgorithm::Status SearchAlgorithm::calcNextDirsPositionIdentification(Vector &cv, const Dir &cd, Dirs &nextDirsKnown, Dirs &nextDirCandidates, int &matchCount)
{
	Vector ans;
